...
画面下部の「議事録を反映」ボタンは**その場の表示だけ**で、他の人には共有されません。
**全関係者に反映する**には、データを編集して push します（push すると数分で自動再公開）。

```bash
# 1) js/data.js を編集（KPI・進捗・タスク・時系列・3テーマ）
# 2) 変更を公開
git add -A
git commit -m "進捗更新：2026/06/XX 〇〇"
git push
