...
- **議事録を反映 → 全員に即時共有**：本文を貼り付けて「反映」すると、時系列（全文付き）とタスクに追加され、Supabase 設定時は**全員の画面へリアルタイム同期**されます。
...
> 共有を有効にするには Supabase の初期設定（1回・無料・約3分）が必要です。手順：[SETUP_SUPABASE.md](SETUP_SUPABASE.md)
> 未設定（🟡 この端末のみ）の場合、変更はその端末のブラウザ内にのみ保存されます。

### 方法2：データを直接編集して公開（恒久的な初期値を変える場合）
```bash
# js/data.js（window.DEFAULT_DATA）を編集 → 公開
git add -A && git commit -m "進捗更新：2026/06/XX 〇〇" && git push
# → 数分後 https://nagaseriku-cmyk.github.io/progress-dashboard/ に反映
```
...
├── js/
│   ├── config.js           … ★Supabase 設定（共有保存先）
│   ├── data.js             … 初期データ（window.DEFAULT_DATA／議事録全文を内包）
│   └── app.js              … 描画・共有データ層・リアルタイム同期・モーダル
├── SETUP_SUPABASE.md       … 「全員に即時共有」を有効にする手順
├── package.json            … 開発サーバ（任意・Vite）
├── .gitignore
└── README.md
```

- **共有保存先を設定**するときは `js/config.js`（→ [SETUP_SUPABASE.md](SETUP_SUPABASE.md)）。
